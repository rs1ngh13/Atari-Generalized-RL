...
def make_env():
    env = gym.make("ALE/BeamRider-v5", render_mode=None) 
    env = NoopResetEnv(env, noop_max=30)  # Random no-ops at start for stochasticity
    env = EpisodicLifeEnv(env)  # End episode on life lost
...
#     env = NoopResetEnv(env, noop_max=30)  # Random no-ops at start for stochasticity
#     env = MaxAndSkipEnv(env, skip=4)  # Skip frames for faster processing
#     env = EpisodicLifeEnv(env)  # End episode on life lost
...
def train():
    ...
    for update in range(1, total_updates + 1):
        ...
        if update % 1000 == 0:
            ...
            plt.tight_layout()
            plt.savefig(f"ppo_training_plot_BR.png")
            plt.close()
