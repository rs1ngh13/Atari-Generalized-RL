...
def record_video_episode(env, agent, tag):
    ...
    video_path = os.path.join(out_dir, f"dd_beamrider_step{tag}.mp4")
...
gym_env = gym.make(beamrider, render_mode="rgb_array")
...
plt.title("Episode Rewards Beamrider (Double_DQN)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.tight_layout()
plt.savefig("episode_rewards_beamrider_double_dqn.png")
plt.show()
