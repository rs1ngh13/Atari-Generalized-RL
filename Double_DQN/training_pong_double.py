...
def record_video_episode(env, agent, tag):
    ...
    video_path = os.path.join(out_dir, f"dd_pong_step{tag}.mp4")
...
gym_env = gym.make(pong, render_mode="rgb_array")
...
plt.title("Episode Rewards Pong (Double_DQN)")
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.tight_layout()
plt.savefig("episode_rewards_pong_double_dqn.png")
plt.show()
