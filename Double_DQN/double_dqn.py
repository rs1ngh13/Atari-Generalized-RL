...
class Double_DQN:
    ...
    def update_q_network(self):
        ...
        with torch.no_grad():
            next_q_vals_policy = self.policy_net(sample_next_states)
            next_actions = next_q_vals_policy.argmax(dim=1, keepdim=True)

            next_q_vals_target = self.target_net(sample_next_states)
            selected_q_vals = next_q_vals_target.gather(1, next_actions).squeeze()

            target_q_vals = sample_rewards + (1 - sample_dones) * self.gamma * selected_q_vals
