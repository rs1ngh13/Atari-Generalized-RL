...
from transfer_learning_double_dqn import DoubleDQN
...
def record_video_episode(env, agent, game, tag):
    ...
    video_path = os.path.join(out_dir, f"double_{game}_step{tag}.mp4")
...
for step in range(1, total_steps+1):
    ...
    if random.random() < epsilon:
        ...
        action = ag.decide_action(np.array(obs), game)

    next_obs, reward, terminated, truncated, _ = env.step(action)
    done = terminated or truncated

    ag.memories[game].push(obs, action, reward, next_obs, done)
    total_reward[game] += reward
    observations[game]   = next_obs

    if step >= begin_learning and len(ag.memories[game]) >= batch_size:
        ag.update_q_network(game)
    ...
    if step in [500000,1000000,1500000,2000000,2500000,3000000,3500000,4000000,4500000,5000000]:
        for g in envs:
            record_video_episode(envs[g], ag, g, f"step_{step}")
    ...
    if done:
        ...
        if episodes[game] % print_every == 0:
            ...
            print("-------------------------------------------------------")
            print(f"Step: {step}, {game.capitalize()} Ep: {episodes[game]}")
            print(f"Avg Reward (last100): {avg:.2f}")
...
ag.save("models/multihead_transfer_proportional_double_dqn.pth")
print("Saved proportional-scheduled multi-task model to models/multihead_transfer_proportional.pth")

for g, r in rewards_per_episode.items():
    plt.plot(range(1,len(r)+1), r, label=g)
plt.title("Episode Rewards Multi-Game Proportional Scheduling")
plt.xlabel("Episode count per game")
plt.ylabel("Clipped Reward")
plt.legend()
plt.tight_layout()
plt.savefig("episode_rewards_proportional_double_dqn.png")
plt.show()

for g, r in rewards_per_episode.items():
    plt.plot(range(1, len(r)+1), r)
    plt.title(f"Episode Rewards — {g.capitalize()} (Double DQN)")
    plt.xlabel("Episode")
    plt.ylabel("Clipped Reward")
    plt.tight_layout()
    plt.savefig(f"episode_rewards_{g.lower()}_double_dqn.png")
    plt.close()